...
import { createWalletRequestSchema } from "@/lib/application/api-validation";
...
export async function GET(request: Request) {
  try {
    ...
    const wallets = await getWorkspaceApplicationService().listWallets(context);
...
export async function POST(request: Request) {
  try {
    const context = await getAuthService().resolve(request);
    const input = createWalletRequestSchema.parse(await readJsonBody(request));
    const result = await getWorkspaceApplicationService().createWallet(
      context,
      input,
